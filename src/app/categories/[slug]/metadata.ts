...
const toAbs = (u?: string) => (u && /^https?:\/\//i.test(u)) ? u : (u ? `https://phimimg.com/${u.replace(/^\/+/, "")}` : "");
...
export async function generateMetadata(
  ...
  try {
    ...
    data = res?.data ?? {}; seo = data?.seoOnPage ?? {};
  } catch {}

  const t = (seo?.titleHead?.trim?.()) || (data?.titlePage?.trim?.()) || pretty(normalized) || "Danh sách phim";
  const desc = (seo?.descriptionHead?.trim?.()) || `Xem phim ${pretty(normalized)||"hay"} online miễn phí, chất lượng HD, cập nhật nhanh.`;

  const ogImages: string[] = Array.isArray(seo?.og_image) ? seo.og_image.map(toAbs).filter(Boolean) : [];
  ...
  return {
    ...
    openGraph: {
      type: "website",
      url, title: t, description: desc, images, siteName: "Phim ngay",
    },
    twitter: {
      card: "summary_large_image",
      title: t, description: desc, images,
    },
  };
}
